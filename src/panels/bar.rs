...
async fn run_monitor(
    ...
    struct Term {
        tid: TermId,
        term_ev_rx: UnbRx<(TermId, TermEvent)>,
        sizes: tui::Sizes,
    ...
    let mut try_run = async || -> anyhow::Result<std::convert::Infallible> {
        ...
        let init_term = async |args, envs, id_prefix| {
            let mut term_upd_tx = term_upd_tx.clone();
            let (term_ev_tx, mut term_ev_rx) = unb_chan();

            let tid = TermId::from_str(&format!("{id_prefix}-{}", monitor.name));

            term_upd_tx.try_emit(TermMgrUpdate::SpawnPanel(SpawnTerm {
                term_id: tid.clone(),
                extra_args: args,
                extra_envs: envs,
                term_ev_tx,
        ...
        let (mut bar, (mut menu, mut watcher_stream)) = futures::future::try_join(
            ...
            async {
                let menu: Term = init_term(
                    ...
                    vec![("BAR_MENU_WATCHER_SOCK".into(), watcher_sock_path.into())],
                    "MENU",
                )
                .await?;
        ...
        loop {
            ...
            let upd = tokio::select! {
                Some((_, ev)) = bar.term_ev_rx.next() => Upd::Term(TermKind::Bar, ev),
                Some((_, ev)) = menu.term_ev_rx.next() => Upd::Term(TermKind::Menu, ev),
            ...
            match upd {
                Upd::BarTui => {
                    ...
                    let mut emit =
                        |tupd| term_upd_tx.emit(TermMgrUpdate::TermUpdate(bar.tid.clone(), tupd));
                    emit(TermUpdate::Print(buf));
                    emit(TermUpdate::Flush);
                }
            ...
            if resize_menu {
                ...
                }) = show_menu
                {
                    ...
                    if tui_size_cache != menu.sizes.cell_size {
                        ...
                        term_upd_tx.emit(TermMgrUpdate::TermUpdate(
                            menu.tid.clone(),
