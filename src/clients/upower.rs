...
pub fn connect() -> impl Stream<Item = EnergyState> {
...
async fn run(tx: mpsc::Sender<EnergyState>) -> anyhow::Result<()> {
    ...
    let updates = futures::stream::empty()
        .merge(
            device
                .receive_energy_rate_changed()
                .await
                .then(|opt| async move { opt.get().await })
                .filter_map(|it| it.ok_or_log("on energy rate"))
                .map(Upd::Rate),
        )
        .merge(
            device
                ...
                .await
                .then(|opt| async move { opt.get().await })
                .filter_map(|it| it.ok_or_log("on percentage"))
                .map(Upd::Percentage),
        )
        .merge(
            device
                ...
                .await
                .then(|opt| async move { opt.get().await })
                .filter_map(|it| it.ok_or_log("on IsPresent"))
                .map(Upd::ShouldShow),
        )
        .merge(
            device
                ...
                .await
                .then(|opt| async move { opt.get().await })
                .filter_map(|it| it.ok_or_log("on battery state"))
                .map(Upd::BState),
        );
    tokio::pin!(updates);
    ...
    while let Some(update) = updates.next().await {
        ...
        tx.send(state.clone())
            .await
            .ok_or_log("Failed to send upower update");
    }
    Ok(())
}
